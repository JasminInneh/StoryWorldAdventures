import React from "react";
import { Container } from "react-bootstrap";
import featuredStories from "../../styles/featuredStories.css";

export const FeaturedStories = () => {
  return (
    <div className="featured container-fluid p-5">
      <h2>Explore Our Features</h2>
      <div className="row mt-5">
        <div class="col-md-4 d-flex justify-content-center hover-grow">
          <div class="card" style={{ width: "18rem;" }}>
            <img
              class="fixed-size-img"
              src="https://i.pinimg.com/736x/ac/16/92/ac169260ec89a74eb92eb49fbeb80837.jpg"
              alt="..."
            />
            <div class="card-body">
              <h5 class="card-title">Word Bank</h5>
              <p class="card-text">
                In Story World Adventures, kids can save new words to their
                "Word Bank" for later review. This helps them learn and remember
                words they discover while reading, making their adventure more
                fun and educational.
              </p>
              <a href="#" class="btn btn-success">
                See word bank
              </a>
            </div>
          </div>
        </div>
        <div class="col-md-4 d-flex justify-content-center hover-grow">
          <div class="card" style={{ width: "18rem;" }}>
            <img
              class="fixed-size-img"
              src="https://img-v3.deepdreamgenerator.com/2291831/md_wxbgcu_3d23c06f437fa000c4ddaba78eac917bf9345e2e.jpg"
              alt="..."
            />
            <div class="card-body">
              <h5 class="card-title">Story Explorer</h5>
              <p class="card-text">
                Explore magical tales tailored for kids aged 5-9 in the Story
                Explorer. Discover exciting adventures, friendly characters, and
                valuable lessons with interactive previews, ratings, and
                personalized recommendations.
              </p>
              <a href="#" class="btn btn-success">
                Explore
              </a>
            </div>
          </div>
        </div>
        <div class="col-md-4 d-flex justify-content-center hover-grow">
          <div class="card" style={{ width: "18rem;" }}>
            <img
              class="fixed-size-img"
              src="data:image/png;base64,"
              alt="..."
            />
            <div class="card-body">
              <h5 class="card-title">Favorites Section</h5>
              <p class="card-text">
                Kids can save their favorite books in the Favorites Section of
                Story World Adventures. Easily revisit beloved adventures and
                magical tales anytime, fostering a love for reading and
                personalized storytelling experiences.
              </p>
              <a href="#" class="btn btn-success">
                Favorites
              </a>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default FeaturedStories;
